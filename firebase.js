...
let auth=null,db=null;if(isFirebaseConfigured()){const app=initializeApp(CONFIG.firebase);auth=getAuth(app);db=getFirestore(app)}
export const firebaseEnabled=isFirebaseConfigured();export const firebaseAuth=auth;export const firebaseDb=db;
export const authState=cb=>auth?onAuthStateChanged(auth,cb):cb(null);
export const login=(e,p)=>auth?signInWithEmailAndPassword(auth,e,p):Promise.reject(new Error('Firebase no está configurado.'));
export const logout=()=>auth?signOut(auth):Promise.resolve();
const clean=o=>Object.fromEntries(Object.entries(o).filter(([,v])=>v!==undefined));
const qPeriod=new Set(['cash','hours','invoices','localExpenses','liquidations','providerEntries','audit']);
export async function remoteList(name,period=null){if(!db)return[];let ref=collection(db,name);let q=period&&qPeriod.has(name)?query(ref,where('period','==',period)):ref;const s=await getDocs(q);return s.docs.map(d=>({id:d.id,...d.data()}))}
...
export async function remoteSet(name,id,data){if(!db)throw new Error('Firebase no está configurado.');await setDoc(doc(db,name,id),clean({...data,updatedAt:serverTimestamp()}),{merge:true});return{id,...data}}
...
export async function remoteDelete(name,id){if(!db)throw new Error('Firebase no está configurado.');await updateDoc(doc(db,name,id),{deleted:true,deletedAt:serverTimestamp(),trashExpiresAt:new Date(Date.now()+30*86400000),updatedAt:serverTimestamp()})}
