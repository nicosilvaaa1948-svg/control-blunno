...
function agentSay(html,who='bot'){
  ...
  try{const h=JSON.parse(localStorage.getItem(key)||'[]');h.push({who,html,at:now()});localStorage.setItem(key,JSON.stringify(h.slice(-120)))}catch(e){}
  if(!box)return;
  box.insertAdjacentHTML('beforeend',`<div class="agent-message ${who}">${html}</div>`);
  box.scrollTop=box.scrollHeight;
}
function rememberAgentPerson(p){
  agentPerson=p;localStorage.setItem('blunno-agent-person',p);
  const el=$('#agentCurrentPerson');if(el)el.textContent=p;
  agentSay(`<b>Agente BLUNNO</b><span>Perfecto, ${esc(p)}. Ya tengo tu responsable. Podés hablarme o escribirme en lenguaje normal.</span>`);
}
function agentText(){
...
function parseMoney(text){
  ...
  const matches=[...t.matchAll(/(?:\b)(\d{1,3}(?:[.\s]\d{3})*(?:,\d{1,2})?|\d+(?:,\d{1,2})?)(?:\s*(?:pesos|peso|ars))?/gi)];
  if(!matches.length)return null;
  const raw=matches[matches.length-1][1].replace(/\s/g,'').replace(/\./g,'').replace(',','.');
...
function detectPeriod(text){
  ...
  if(/este mes|mes actual|actual/.test(l))return period;
  if(/mes pasado|ultimo mes|último mes/.test(l))return prevPeriod(period);
  if(/proximo mes|pr[oó]ximo mes|siguiente mes/.test(l))return nextPeriod(period);
  return period;
}
function dateFromAgent(text, fallback=today()){
  ...
  if(l.includes('ayer')){const x=new Date(`${today()}T12:00:00`);x.setDate(x.getDate()-1);return isoDate(x.getFullYear()+'-'+String(x.getMonth()+1).padStart(2,'0'),x.getDate());}
  return fallback;
}
...
function agentFindProvider(text){
  ...
  const exact=rows.find(x=>normalizeAgentText(x.name)===l);if(exact)return exact;
  const hits=rows.filter(x=>l.includes(normalizeAgentText(x.name))||normalizeAgentText(x.name).includes(l)).sort((a,b)=>normalizeAgentText(a.name).length-normalizeAgentText(b.name).length);
  return hits[0]||null;
}
function agentFindEmployee(text, scopeBranch='General'){
  ...
  const hits=rows.filter(x=>l.includes(normalizeAgentText(x.name))||normalizeAgentText(x.name).includes(l));
  return hits.sort((a,b)=>normalizeAgentText(a.name).length-normalizeAgentText(b.name).length)[0]||null;
}
...
function agentList(items, empty='No hay registros.'){return items.length?items.slice(0,12).map(x=>`<small class="agent-line">${esc(x)}</small>`).join(''):`<small class="agent-line">${empty}</small>`}
function showAgentConfirmation(label){
  ...
  document.querySelector('[data-agent-confirm]')?.addEventListener('click',confirmAgent);
  document.querySelector('[data-agent-cancel]')?.addEventListener('click',()=>{pendingAgent=null;agentSay('<span>Acción cancelada. No se modificó ningún dato.</span>')});
}
function makePending(type,data,missing,preview){pendingAgent={type,data,missing,preview};showAgentConfirmation(`Preparar ${type==='invoice'?'factura':type==='expense'?'gasto':type==='investment'?'inversión':type==='cash'?'movimiento de caja':type==='hours'?'horas':type==='employee'?'empleado':'recordatorio'}`)}
async function processAgent(text){
  ...
  const navKey=Object.keys(nav).find(k=>new RegExp(`\\b${k}\\b`).test(l));
  if(/^(abr[ií]|mostrame|mostrar|lleva(me|nos)|ir a|entra(r)?|quiero ver|ver)\b/.test(l)&&navKey){agentOpen(nav[navKey]);return;}
  if(/^(ayuda|que podes|que puedes|comandos|como te uso|que haces)/.test(l)){agentSay('<b>Agente BLUNNO</b><span>Puedo abrir sectores, consultar y comparar datos, buscar proveedores/facturas, revisar caja, gastos, inversiones, personal y horas, preparar recordatorios, preparar controles de cierre y cargar movimientos. Para cualquier modificación siempre te muestro los datos y pido confirmación.</span>');return;}
  ...
  if(/cierre/.test(l)&&!/(cerrar|confirmar cierre|hacer cierre)/.test(l)){
    const s=Store.summary(scopePeriod,scopeBranch), pending=Store.list('invoices',scopePeriod,scopeBranch).filter(x=>x.status!=='paid');
    agentQueryResult('Control de cierre',`${periodLabel(scopePeriod)} · ${esc(scopeBranch)} · ingresos ${money(s.income)} · gastos ${money(s.totalExpenses)} · proveedores ${money(s.providers)} · inversiones ${money(s.investment)} · horas ${number(s.hours)} · resultado ${money(s.result)}. ${pending.length?`Hay ${pending.length} facturas pendientes.`:'No hay facturas pendientes.'}`);return;
  }
  if(/facturas?|proveedores?/.test(l)&&/(pendiente|debo|deben|falta pagar|por pagar|sin pagar)/.test(l)){
    const rows=Store.list('invoices',scopePeriod,scopeBranch).filter(x=>x.status!=='paid');const total=rows.reduce((a,x)=>a+Number(x.amount||0)-Number(x.paidAmount||0),0);
    agentQueryResult('Facturas pendientes',`${rows.length} registros por ${money(total)} en ${periodLabel(scopePeriod)} · ${esc(scopeBranch)}.${agentList(rows.map(x=>`${x.provider} · ${money(Number(x.amount||0)-Number(x.paidAmount||0))} · ${dateLabel(x.operationDate||x.date)}`))}`);return;
  }
  if(/proveedor/.test(l)&&/(buscar|busca|tenemos|existe|mostra|mostrar|cuanto|cuanto le|historial)/.test(l)){
    const p=agentFindProvider(l.replace(/.*?proveedor\s*/,'').trim());
    if(!p){agentQueryResult('Proveedor','No encontré ese proveedor en el catálogo.');return;}
    ...
    agentQueryResult(p.name,`${inv.length} facturas en ${periodLabel(scopePeriod)} · total ${money(total)}.${agentList(inv.map(x=>`${x.provider} · ${money(x.amount)} · ${dateLabel(x.operationDate||x.date)}`))}`);return;
  }
  if(/(cuanto|total|resumen|resultado|gastamos|ingresos|gastos|balance|neto)/.test(l)&&!/(factura|proveedor)/.test(l)){
    const s=Store.summary(scopePeriod,scopeBranch);
    agentQueryResult(`Resumen ${periodLabel(scopePeriod)}`,`Perfil: ${esc(scopeBranch)} · ingresos ${money(s.income)} · gastos caja ${money(s.cashExpense)} · gastos local ${money(s.localExpense)} · proveedores ${money(s.providers)} · inversiones ${money(s.investment)} · personal ${money(s.salary)} · resultado ${money(s.result)}.`);return;
  }
  if(/caja|saldo/.test(l)&&/(saldo|ingreso|egreso|gasto|resto|final)/.test(l)){
    ...
    agentQueryResult('Caja',`${periodLabel(scopePeriod)} · ${esc(scopeBranch)} · ingresos ${money(inc)} · gastos ${money(exp)} · movimientos ${rows.length}. Saldo neto de movimientos: ${money(inc-exp)}.`);return;
  }
  if(/(horas|empleados|personal)/.test(l)&&/(cu[aá]nt|total|resumen|carg|trabaj)/.test(l)){
    const s=Store.summary(scopePeriod,scopeBranch), rows=Store.list('hours',scopePeriod,scopeBranch);agentQueryResult('Horas',`${number(s.hours)} horas registradas en ${periodLabel(scopePeriod)} · ${esc(scopeBranch)} · ${rows.length} registros.`);return;
  }
  ...
  if(/(crear|cargar|agregar|registrar|anotar|poneme|guard[aá])/.test(l)&&/(recordatorio|tarea)/.test(l)){
    const title=text.replace(/.*?(recordatorio|tarea)\s*(para)?\s*/i,'').trim()||'Nueva tarea';
    const d=dateFromAgent(text,today()), time=(text.match(/\b([01]?\d|2[0-3]):([0-5]\d)\b/)||[]).slice(1).join(':');
    makePending('task',{title:title.replace(/\b(mañana|hoy|urgente)\b/gi,'').trim()||title,dueDate:d,dueTime:time,priority:agentPriority(text),responsible:agentPerson,status:'pending',repeat:'none',notes:'Creado por Agente BLUNNO'},[],`Crear recordatorio para ${agentPerson} el ${dateLabel(d)}${time?' a las '+time:''}.`);return;
  }
  if(/(cargar|registrar|agregar|anotar|guard[aá])/.test(l)&&/factura/.test(l)){
    const provider=agentFindProvider(l),amount=parseMoney(text),date=dateFromAgent(text,today()),missing=[];if(!provider)missing.push('proveedor');if(!amount)missing.push('importe');if(scopeBranch==='General')missing.push('sucursal concreta');
    const b=scopeBranch==='General'?'':scopeBranch;makePending('invoice',{provider:provider?.name||'',number:'',operationDate:date,loadDate:now(),dueDate:date,branch:b,amount:amount||0,paidAmount:0,status:'pending',period:date.slice(0,7),notes:'Cargada por Agente BLUNNO'},missing,`${provider?.name||'Proveedor pendiente'} · ${money(amount||0)} · ${esc(b||'sucursal pendiente')} · movimiento ${dateLabel(date)}.`);return;
  }
  if(/(cargar|registrar|agregar|anotar|guard[aá])/.test(l)&&/gasto/.test(l)){
    const amount=parseMoney(text),date=dateFromAgent(text,today()),missing=[];if(!amount)missing.push('importe');if(scopeBranch==='General')missing.push('sucursal concreta');
    const category=CONFIG.expenseCategories.find(c=>l.includes(normalizeAgentText(c)))||'Otros gastos';const concept=text.replace(/.*?gasto\s*/i,'').replace(/\$?\s*[\d.]+(?:,\d+)?\s*(pesos|peso|ars)?/i,'').trim()||'Gasto cargado por Agente BLUNNO';
    makePending('expense',{date,period:date.slice(0,7),branch:scopeBranch==='General'?'':scopeBranch,category,concept,amount:amount||0,document:'',notes:'Cargado por Agente BLUNNO'},missing,`${category} · ${concept} · ${money(amount||0)} · ${esc(scopeBranch)}.`);return;
  }
  if(/(cargar|registrar|agregar|anotar|guard[aá])/.test(l)&&/inversi[oó]n/.test(l)){
    const amount=parseMoney(text),date=dateFromAgent(text,today()),missing=[];if(!amount)missing.push('importe');if(scopeBranch==='General')missing.push('sucursal concreta');
    makePending('investment',{date,period:date.slice(0,7),branch:scopeBranch==='General'?'':scopeBranch,concept:text,amount:amount||0,document:'',notes:'Cargada por Agente BLUNNO'},missing,`Inversión · ${money(amount||0)} · ${esc(scopeBranch)}.`);return;
  }
  if(/(cargar|registrar|agregar|anotar|guard[aá])/.test(l)&&/(ingreso|caja)/.test(l)){
    const amount=parseMoney(text),date=dateFromAgent(text,today()),missing=[];if(!amount)missing.push('importe');if(scopeBranch==='General')missing.push('sucursal concreta');
    const type=/egreso|gasto|pago/.test(l)?'expense':'income';makePending('cash',{date,period:date.slice(0,7),branch:scopeBranch==='General'?'':scopeBranch,type,concept:text,amount:amount||0,expected:amount||0,notes:'Cargado por Agente BLUNNO'},missing,`${type==='income'?'Ingreso':'Egreso'} de caja · ${money(amount||0)} · ${esc(scopeBranch)}.`);return;
  }
  if(/(cargar|registrar|agregar|anotar|guard[aá])/.test(l)&&/horas?/.test(l)){
    const emp=agentFindEmployee(l,scopeBranch),amount=parseMoney(text),date=dateFromAgent(text,today()),missing=[];const hourMatch=l.match(/(\d+(?:[,.]\d+)?)\s*horas?/);const hrs=hourMatch?Number(hourMatch[1].replace(',','.')):null;if(!emp)missing.push('empleado');if(!hrs)missing.push('cantidad de horas');
    makePending('hours',{employee:emp?.name||'',date,hours:hrs||0,advance:0,merchandise:0,holiday:'no',notes:'Cargadas por Agente BLUNNO'},missing,`${emp?.name||'Empleado pendiente'} · ${hrs||0} horas · ${dateLabel(date)}.`);return;
  }
  if(/(agregar|crear|alta|nuevo)/.test(l)&&/(empleado|personal)/.test(l)){
    const name=text.replace(/.*?(empleado|personal)\s*(nuevo|nueva|que se llama|llamado|llamada)?\s*/i,'').trim();const missing=[];if(!name||name.length<2)missing.push('nombre del empleado');if(scopeBranch==='General')missing.push('sucursal concreta');
    makePending('employee',{name:name||'',branch:scopeBranch==='General'?'':scopeBranch,role:'',hourlyRate:0,active:true},missing,`Crear empleado ${name||'(falta nombre)'} en ${scopeBranch}.`);return;
  }
  agentSay('<b>Agente BLUNNO</b><span>No encontré una acción segura para esa frase. Probá, por ejemplo: “abrime proveedores”, “cuánto gastamos en septiembre en Mendiolaza”, “mostrame las facturas pendientes”, “cargá una factura de Aguilera por 125000”, “agregá 8 horas de Agus”, o “creá un recordatorio para mañana a las 10”.</span>');
}
async function confirmAgent(){
  if(!pendingAgent)return;
  try{
    const {type,data}=pendingAgent;
    const collection={task:'tasks',invoice:'invoices',investment:'investments',expense:'expenses',cash:'cash',hours:'hours',employee:'employees'}[type];
    if(!collection)throw new Error('Acción no disponible.');
    if(['invoice','expense','investment','cash'].includes(type)&&data.branch==='')throw new Error('El Agente necesita una sucursal concreta para guardar este movimiento. Elegí Mendiolaza, Bodereau, Derqui o Unquillo.');
    if(type==='hours'){
      const emp=Store.list('employees').find(x=>normalizeAgentText(x.name)===normalizeAgentText(data.employee));
      if(!emp)throw new Error('No se encontró el empleado.');
      data.employeeId=emp.id;data.branch=emp.branch;data.period=String(data.date).slice(0,7);data.salaryCost=Number(data.hours||0)*Number(emp.hourlyRate||0);
    }
    if(type==='employee')data.createdByAgent=true;
    const row=await Store.add(collection,data,agentPerson,'Acción confirmada por Agente BLUNNO');
    pendingAgent=null;refresh();
    setTimeout(()=>{agentSay(`<b>✓ Acción ejecutada correctamente</b><span>${esc(collection)} quedó guardado con responsable ${esc(agentPerson)}, fecha/hora y auditoría completa.</span><button class="secondary-button" data-undo-agent="${row.id}" data-undo-collection="${collection}">↶ Deshacer esta acción</button>`);document.querySelector('[data-undo-agent]')?.addEventListener('click',()=>undoAgent(collection,row.id));},30);
...
function startSpeech(){
  ...
  if(!SR){toast('Este navegador no tiene reconocimiento de voz habilitado. Probá Chrome o Edge.',false);return;}
  if(speechRecognition){try{speechRecognition.abort()}catch(e){}speechRecognition=null;return;}
  const r=new SR();speechRecognition=r;r.lang='es-AR';r.continuous=false;r.interimResults=true;r.maxAlternatives=3;
  const btn=$('#agentMic');
  if(btn){btn.textContent='🔴';btn.title='Escuchando… hablá ahora';}
  r.onresult=e=>{
    let finalText='';for(let i=e.resultIndex;i<e.results.length;i++)finalText+=e.results[i][0].transcript+' ';
    if($('#agentInput'))$('#agentInput').value=finalText.trim();
  };
  r.onend=()=>{if(btn){btn.textContent='🎙';btn.title='Dictar'};speechRecognition=null;const t=$('#agentInput')?.value.trim();if(t)agentText()};
  r.onerror=e=>{if(btn){btn.textContent='🎙';btn.title='Dictar'};speechRecognition=null;if(e.error!=='aborted')toast('No se pudo interpretar el audio. Hablá nuevamente.',false)};
  try{r.start()}catch(e){speechRecognition=null;if(btn){btn.textContent='🎙'};toast('No se pudo iniciar el micrófono.',false)}
}

async function init(){
 currentOperator=localStorage.getItem('blunno-operator')||'';agentPerson=localStorage.getItem('blunno-agent-person')||currentOperator||'';branch=localStorage.getItem('blunno-branch')||'General';setOperator(currentOperator);connection();
 await Store.sync();
 globalWire();refresh();
}
init();
