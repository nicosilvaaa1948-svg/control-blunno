import { CONFIG, periodLabel, nextPeriod, prevPeriod, dateLabel, dateTimeLabel, today, now, daysInPeriod, isoDate, money, number } from "./config.js";
...
function showProviderHistory(provider){
 const inv=Store.list('invoices',period,branch).filter(x=>x.provider===provider).sort((a,b)=>String(b.operationDate||'').localeCompare(String(a.operationDate||''));
 const total=inv.reduce((s,x)=>s+Number(x.amount||0),0);
 $('#detailTitle').textContent=`Historial · ${provider}`;
 $('#detailBody').innerHTML=`<div class="panel"><div class="mini-kpis">${money(total)} · ${inv.length} facturas</div>${inv.length?`<div class="table-wrap"><table><thead><tr><th>Factura</th><th>Fecha movimiento</th><th>Fecha carga</th><th>Sucursal</th><th>Importe</th><th>Estado</th></tr></thead><tbody>${inv.map(x=>`<tr><td>${esc(x.number||'—')}</td><td>${dateLabel(x.operationDate||x.date)}</td><td>${dateTimeLabel(x.loadDate||x.createdAt)}</td><td>${esc(x.branch)}</td><td>${money(x.amount)}</td><td>${esc(x.status||'pendiente')}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty-block">No hay facturas de este proveedor en el período/perfil seleccionado.</div>'}<div class="modal-footer"><button class="primary-button" id="providerHistoryPdf">Descargar PDF</button></div></div>`;
 $('#detailModal').classList.remove('hidden');
 $('#providerHistoryPdf').onclick=()=>generateProviderHistoryPDF(provider,inv);
}
